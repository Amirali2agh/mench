...
# Setup a dummy FastAPI app to test the websocket router in isolation
test_app = FastAPI()
...
def test_websocket_queue_flow():
    ...
    matching and automatic cleanup on disconnect.
    """
    ...
    with patch("app.api.websocket.queue.QueueService", new_callable=AsyncMock) as mock_service, \
        ...
        # Act: Open a simulated WebSocket connection with expected query parameters
        with client.websocket_connect("/ws/queue?playerId=p1&playerName=Amir&playerAvatarUrl=http://avatar&playerCount=2") as websocket:
            # The context manager automatically closes the socket upon block exit
            pass
        
        # Assert: Verify correct parameters were forwarded to the queue service
        ...
        # Assert: Verify automatic cleanup is triggered on connection drop
        mock_service.remove_from_queue.assert_called_once_with("p1", 2)
