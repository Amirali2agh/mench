// front/src/components/GameBoard4P.tsx
// Standalone board for exactly 4 players, built from the ground up.
// Seating (clockwise, no diagonal neighbours):
//   Red bottom-left (0), Blue bottom-right (1),
//   Green top-right (2), Yellow top-left (3).
// Backend player index == corner index (identity mapping, no swapping).
...
import {
  ...
  safeTrackPositions,
  circularTrack,
} from '../utils/boardCoordinates4p';
import Dice from './Dice';
import type { ChatMessage } from '../hooks/useMenschSocket';

interface GameBoard4PProps {
  gameState: GameState;
...
// Corner indices: 0 = red BL, 1 = blue BR, 2 = green TR, 3 = yellow TL.
// Panels are laid out so the local player's seat is always at the bottom.
const getPanelPosition = (vIdx: number): 'tl' | 'tr' | 'bl' | 'br' | null => {
  const map: Record<number, 'tl' | 'tr' | 'bl' | 'br'> = {
    3: 'tl',  // yellow -> top-left
    2: 'tr',  // green  -> top-right
    0: 'bl',  // red    -> bottom-left
    1: 'br',  // blue   -> bottom-right
  };
  return map[vIdx] || null;
};

export const GameBoard4P: React.FC<GameBoard4PProps> = ({
  gameState,
  localPlayerId,
  onRollDice,
  onMovePiece,
  ...
  const getColorForIdx = (vIdx: number): string => {
  ...
  const renderPlayerPanel = (playerIdx: number) => {
    const pos = getPanelPosition(playerIdx);
    if (!pos || !players[playerIdx]) return null;
    const p = players[playerIdx];
    const color = getColorForIdx(playerIdx);
  ...
  const tlPlayer = players[3] ? 3 : -1;
  const trPlayer = players[2] ? 2 : -1;
  const blPlayer = players[0] ? 0 : -1;
  const brPlayer = players[1] ? 1 : -1;
    ...
    <div className="h-screen w-screen flex flex-col bg-[#F9F6F0] font-sans overflow-hidden select-none">
      ...
      <main className="flex-1 flex flex-col overflow-hidden px-2">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-1 min-h-0">
          ...
          <div ref={boardRef} className="relative w-[min(88vw,78vh)] aspect-square max-w-[480px] bg-[#FAF4E6] rounded-[clamp(16px,3vmin,28px)] p-[clamp(8px,1.5vmin,16px)] shadow-[0_16px_36px_rgba(0,0,0,0.08),inset_0_2px_6px_rgba(255,255,255,0.6)] border-[clamp(2px,0.4vmin,4px)] border-[#E8D3B0] flex items-center justify-center">
            <div className="relative w-full h-full">
              <div className="grid grid-cols-11 grid-rows-11 gap-[0.35vmin] w-full h-full relative">
                ...
                {/* Yards: TL=yellow, TR=green, BL=red, BR=blue */}
                <div className="col-start-1 col-end-5 row-start-1 row-end-5 p-[0.3vmin]">{renderBaseYard(3)}</div>  {/* TL: yellow */}
                <div className="col-start-8 col-end-12 row-start-1 row-end-5 p-[0.3vmin]">{renderBaseYard(2)}</div>  {/* TR: green */}
                <div className="col-start-1 col-end-5 row-start-8 row-end-12 p-[0.3vmin]">{renderBaseYard(0)}</div>  {/* BL: red */}
                <div className="col-start-8 col-end-12 row-start-8 row-end-12 p-[0.3vmin]">{renderBaseYard(1)}</div>  {/* BR: blue */}
                ...
                {Array.from({ length: 11 }).map((_, rIdx) => {
                  return Array.from({ length: 11 }).map((_, cIdx) => {
                      ...
                      return (
                        <div key={`cell-${rIdx}-${cIdx}`} className="col-start-6 col-end-7 row-start-6 row-end-7 flex items-center justify-center relative">
                          <div className="w-full h-full rounded-lg overflow-hidden relative">
                            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-0 h-0 border-l-[clamp(8px,1.5vmin,18px)] border-r-[clamp(8px,1.5vmin,18px)] border-b-[clamp(8px,1.5vmin,18px)] border-l-transparent border-r-transparent border-b-[#43AA8B]/40" />
                            <div className="absolute right-0 top-1/2 -translate-y-1/2 w-0 h-0 border-t-[clamp(8px,1.5vmin,18px)] border-b-[clamp(8px,1.5vmin,18px)] border-l-[clamp(8px,1.5vmin,18px)] border-t-transparent border-b-transparent border-l-[#277DA1]/40" />
                    ...
                    if (rIdx === 5 && cIdx >= 6 && cIdx <= 9) { customBg = 'bg-[#277DA1]/30'; isHomeStretch = true; }
                    else if (cIdx === 5 && rIdx >= 1 && rIdx <= 4) { customBg = 'bg-[#43AA8B]/30'; isHomeStretch = true; }
                    else if (rIdx === 5 && cIdx >= 1 && cIdx <= 4) { customBg = 'bg-[#F9C74F]/30'; isHomeStretch = true; }
          ...
          {/* Bottom row: players + roll control */}
